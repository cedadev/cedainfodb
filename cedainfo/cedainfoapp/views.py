...
def host_detail(request, host_id):
    ...
    try:
        ...
        return render_to_response('cedainfoapp/host_detail.html',
                                  {'host': host, 'services': services, 'history': history, 'user': request.user})
    except:
...
def fileset_list(request):
    ...
    return list_detail.object_list(
            ...
            extra_context={"totaldu": totaldu, "totalalloc": totalalloc, "totalnum": totalnum,
                           "search": search, "backupinfo": backupinfo}
    )
...
def partition_list(request):
    ...
    if partfilter == 'overfill':
        ...
        for p in partitions:
            ...
            if 100.0 * allocated / (p.capacity_bytes + 1) > 85.0 and p.status != 'Retired': filtered_partitions.append(
                p)
    ...
    # Use the object_list view.
    return render_to_response('cedainfoapp/partition_list.html',
...
def storagesummary(request):
    ...
    sumtable = [{'status': 'Blank', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Allocating', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Allocating_ps', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Closed', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Migrating', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Retired', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                {'status': 'Total', 'npart': 0, 'used': 0, 'allocated': 0, 'allocused': 0, 'sec_allocated': 0,
                 'sec_allocused': 0, 'capacity': 0},
                ]
    index = {}
...
def split_fileset(request):
    ...
    size_in = request.GET.get('size', None)
    if path == None:
        return render_to_response('cedainfoapp/fileset_split.html',
    ...
    try:
        ...
        return render_to_response('cedainfoapp/fileset_split.html', {'path': path, 'size': size_in,
                                                                     'error': 'Fileset split error: %s' %
                                                                              sys.exc_info()[1]})
...
def create_gws_update_request(request, id):
    ...
    if reqid:
        ...
        return render_to_response('error.html', {'error': "no update id generated", 'user': request.user})
