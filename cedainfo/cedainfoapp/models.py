...
class ProblemsMixin(object):
    ...
    def problem_html(self, msg, level=0):
        return '<a href="%s">%s</a><font color="%s">[%s] %s</font>' % (
            self.get_admin_url(),
            self,
            self.problem_colours[level],
            self.problem_levels[level],
            msg,
        )
...
class FilseSetCreationError(Exception):
    pass
...
class Host(models.Model):
    ...
    hostname = models.CharField(
        max_length=512, help_text="Full hostname e.g. machine.badc.rl.ac.uk"
    )
    ip_addr = models.GenericIPAddressField(
        blank=True, null=True, help_text="IP Address"
    )  # TODO check if admin allows no ip_addr field (otherwise default='0.0.0.0')
    serial_no = models.CharField(
        max_length=126, blank=True, help_text="Serial no (if physical machine)"
    )
    po_no = models.CharField(
        max_length=126, blank=True, help_text="Purchase order no (if physical machine)"
    )
    ...
    supplier = models.CharField(
        max_length=126, blank=True, help_text="Hardware supplier (if physical machine)"
    )
    ...
    planned_end_of_life = models.DateField(
        null=True,
        blank=True,
    ...
    ftpmountpoints = models.CharField(
        max_length=512,
        blank=True,
    ...
    capacity = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Rough estimate in Tb only",
    )  # just an estimate (cf Partition which uses bytes from df)
...
class Partition(models.Model, ProblemsMixin):
    ...
    mountpoint = models.CharField(
        blank=True, max_length=1024, help_text="E.g. /disks/machineN", unique=True
    )
    host = models.ForeignKey(
        ...
        blank=True,
        null=True,
        help_text="Host on which this partition resides",
    )
    ...
    def df(self):
        ...
        output = subprocess.Popen(
            ["/usr/local/bin/pan_df", "-B 1024", self.mountpoint],
            stdout=subprocess.PIPE,
        ).communicate()[0]
        lines = output.split("\n")
        if len(lines) == 3:
            dev, blocks_total, blocks_used, blocks_available, used, mount = lines[
                1
            ].split()
        if len(lines) == 4:
            blocks_total, blocks_used, blocks_available, used, mount = lines[2].split()
    ...
    def list_allocated(self):
        ...
        if len(allocs) > 0:
            s += "Primary File Sets (biggest first): "
        for a in allocs:
            s += '<a href="/admin/cedainfoapp/fileset/%s">%s</a> ' % (
                a.pk,
                a.logical_path,
            )

        allocs = FileSet.objects.filter(secondary_partition=self).order_by(
            "-overall_final_size"
        )
        if len(allocs) > 0:
            s += "Secondary File Sets (biggest first): "
        for a in allocs:
            s += '<a href="/admin/cedainfoapp/fileset/%s">%s</a> ' % (
                a.pk,
                a.logical_path,
    ...
    def meter(self):
        ...
        googlechart += "&chd=t:%s,%s|%s,%s|%s,0|%s,0" % (
            filesetsum * scale,
            current_allocated * scale,
            secondaryfilesetsum * scale,
            current_secondary_allocated * scale,
            (used - filesetsum - secondaryfilesetsum) * scale,
            (self.capacity_bytes - used) * scale,
        )
        googlechart += "&chxr=1,0,%s&chds=0,%s" % (
            1.2 * self.capacity_bytes * scale,
            1.2 * self.capacity_bytes * scale,
        )  # add data and axis range
    ...
    def text_meter(self):
        ...
        if self.capacity_bytes < 1000:
            ...
            unit = "PB"
            scale = 0.000000000000001
        return "%5.2f%s used of %5.2f%s (%2.2f%%)" % (
            used * scale,
            unit,
            cap * scale,
            unit,
    ...
    def use_summary(self):
        ...
        for a in prime_allocs:
            ...
            if last_size != None:
                use["prime_alloc_used"] += a.last_size().size
        for a in second_allocs:
            use["second_alloc"] += a.overall_final_size
            last_size = a.last_size()
            if last_size != None:
                use["second_alloc_used"] += a.last_size().size
        use["unallocated"] = (
            self.capacity_bytes - use["prime_alloc"] - use["second_alloc"]
        )
        use["unalloc_used"] = (
            self.used_bytes - use["prime_alloc_used"] - use["second_alloc_used"]
        )
    ...
    __str__.allow_tags = (
        True  # seem to need this in order to use __str__ as one of the fields
    )
...
class AccessStatus(models.Model):
    ...
    comment = models.CharField(
        max_length=1024, help_text="Comment describing this status"
    )
...
class FileSet(models.Model, ProblemsMixin):
    ...
    )  # Additional data still expected (as yet uningested) in bytes
    notes = models.TextField(blank=True)
    partition = models.ForeignKey(
        ...
        blank=True,
        null=True,
        limit_choices_to={"status": "Allocating"},
        help_text="Actual partition where this FileSet is physically stored",
    )
    storage_pot_type = models.CharField(
        max_length=64,
        blank=True,
        default="archive",
    ...
    storage_pot = models.CharField(
        max_length=1024,
        blank=True,
        default="",
        help_text="The directory under which the data is held",
    )
    migrate_to = models.ForeignKey(
        Partition,
        on_delete=models.PROTECT,
        blank=True,
        null=True,
        help_text="Target partition for migration",
        related_name="fileset_migrate_to_partition",
    )
    secondary_partition = models.ForeignKey(
        Partition,
        on_delete=models.PROTECT,
        blank=True,
        null=True,
    ...
    complete_date = models.DateField(
        null=True, blank=True, help_text="Date when fileset was set to complete."
    )
    primary_on_tape = models.BooleanField(
        default=False, help_text="Primary media storage on tape."
    )
    ...
    def secondary_storage_path(self):
        if self.secondary_partition:
            return os.path.normpath(
                self.secondary_partition.mountpoint
                + "/backup/"
                + self.storage_pot_type
                + "/"
                + self.storage_pot
    ...
    def migrate_spot(self, do_audit=True):
        assert (
            self.storage_pot != ""
        ), "can't migrate a spot does not already exists in the db"
        ...
        assert os.path.islink(
            self.logical_path
        ), "can't migrate a spot if logical path is not a link"
        ...
        if (
            os.readlink(self.logical_path) != self.migrate_path()
        ):  # may be tring again after failing
        ...
        if do_audit:
            ...
            DOC.write(
                """This storage directory has been migrated and can be deleted.
    ...
    def secondary_copy_command(self):
        ...
        if not self.secondary_partition:
            return ""
        host = self.partition.host.hostname
        frompath = self.storage_path()
        topath = "badc@%s:%s" % (
            self.secondary_partition.host.hostname,
            self.secondary_storage_path(),
        )
    ...
    def status(self):
        ...
        if not self.partition:
            if self.migrate_to or self.storage_pot:
                ...
                linkpath = os.readlink(self.logical_path)
                if linkpath[0:7] == "/disks/":
                    return '<font color="#bbbb00">New (Can allocate from existing link)</font>'
            else:
    ...
    def make_fileset(self, path, size, on_tape=False):
        ...
        if len(filesets) != 0:
            raise FilseSetCreationError(
                "File set with same logical path already exists."
            )
        ...
        if spottail == "":
            head, spottail = os.path.split(head)
        ...
        try:
            ...
            self.delete()
            raise FilseSetCreationError(
                "Can't chown the storage pot directory (removed spot dir for consistancy)"
            )
        try:
            ...
            self.delete()
            raise FilseSetCreationError(
                "Can't make link (removed spot dir for consistancy)"
            )
        self.save()
    ...
    def split_fileset(self, path, size):
        ...
        if path == self.logical_path:
            raise FilseSetCreationError(
                "Can't split: path identical to current fileset"
            )
        if self.logical_path != path[0 : len(self.logical_path)]:
            raise FilseSetCreationError(
        ...
        if not os.path.isdir(path):
            raise FilseSetCreationError("Can only split on a directory.")
        if os.path.islink(path):
            raise FilseSetCreationError("Can't split on a symlink.")

        # make new fileset with same partition.
        new_fs = FileSet(
            partition=self.partition,
            logical_path=path,
            overall_final_size=size,
    ...
    def backup_summary2(self):
        url = (
            "http://storaged-monitor.esc.rl.ac.uk/storaged_ceda/CEDA_Fileset_Summary_XML.php?fileset=%s;level=top"
            % self.storage_pot
        )
        ...
        if m:
            backupsize, backupfiles, nagg, firstfiletime, lastupdated = (
                int(m.group(1)),
                int(m.group(2)),
                int(m.group(3)),
                m.group(4),
        ...
        return "%s files, %s bytes last stored %s" % (
            backupfiles,
            backupsize,
            lastupdated,
        )
    ...
    def problems():
        ...
        for f in filesets:
            ...
            if not first_time:
                ...
                if (
                    datetime.now() - fssms[len(fssms) - 1].date < timedelta(days=60)
                    and not f.sd_backup
                ):
            ...
            if f.sd_backup:
                ...
                if backup_processed[:3] == "Not":
                    ...
                    date = datetime(
                        int(backup_processed[:4]),
                        int(backup_processed[4:6]),
                        int(backup_processed[6:8]),
                    )
            ...
            if f.migrate_to:
                msgs.append(
                    f.problem_html(
                        "Fileset marked for migration to %s" % f.migrate_to, 3
                    )
                )
    ...
    def du(self):
        ...
        if self.spot_exists():
            ...
            output = subprocess.Popen(
                ["/usr/bin/du", "-sk", "--apparent", self.storage_path()],
                stdout=subprocess.PIPE,
            ).communicate()[0]
            ...
            p1 = subprocess.Popen(
                ["find", self.storage_path(), "-type", "f"], stdout=subprocess.PIPE
            )
            p2 = subprocess.Popen(["wc", "-l"], stdin=p1.stdout, stdout=subprocess.PIPE)
            p1.stdout.close()  # Allow p1 to receive a SIGPIPE if p2 exits.
            nfiles = p2.communicate()[0]

            fssm = FileSetSizeMeasurement(
                fileset=self,
                date=datetime.utcnow(),
                size=int(size) * 1024,
                alloc=self.overall_final_size,
                no_files=int(nfiles),
            )
            fssm.save()
        return
    ...
    def dataentity(self):
        ...
        for de in des:
            if de.logical_path == "":
                return None  # skip if getting to DE with no logical path set
    ...
    def links(self):
        ...
        s = (
            '<a href="/fileset/%s/du">du</a> <a href="http://storaged-monitor.esc.rl.ac.uk/storaged_ceda/CEDA_Fileset_Summary.php?%s">storageD</a>'
    ...
    def last_size(self):
        ...
        try:
            fssm = FileSetSizeMeasurement.objects.filter(fileset=self).order_by(
                "-date"
            )[0]
            return fssm
        except:
...
class DataEntity(models.Model):
    ...
    dataentity_id = models.CharField(
        help_text="MOLES data entity id", max_length=255, unique=True
    )
    ...
    symbolic_name = models.CharField(
        max_length=1024,
        blank=True,
        help_text="Short abbreviation to be used for directory names etc",
    )
    logical_path = models.CharField(
        max_length=1024, blank=True, help_text="Top level of location within archive"
    )
    curation_category = models.ForeignKey(
        CurationCategory,
        on_delete=models.PROTECT,
        null=True,
        blank=True,
    ...
    notes = models.TextField(blank=True, help_text="Additional notes")
    availability_priority = models.BooleanField(
        default=False, help_text="Priority dataset : use highest spec hardware"
    )
    availability_failover = models.BooleanField(
        default=False,
    ...
    access_status = models.ForeignKey(
        AccessStatus, on_delete=models.PROTECT, help_text="Security applied to dataset"
    )
    recipes_expression = models.CharField(max_length=1024, blank=True)
    recipes_explanation = models.TextField(
        blank=True,
    ...
    db_match = models.IntegerField(
        null=True, blank=True, help_text="Admin use only : please ignore"
    )  # id match to "dataset" in old storage db
    ...
    last_reviewed = models.DateField(
        null=True, blank=True, help_text="Date of last dataset review"
    )
    ...
    next_review = models.DateField(
        null=True, blank=True, help_text="Date of next dataset review"
    )
...
class Service(models.Model):
    ...
    host = models.ForeignKey(
        Host,
        on_delete=models.PROTECT,
        help_text="Host machine on which service is deployed",
        null=True,
        blank=True,
    )
    name = models.CharField(max_length=512, help_text="Name of service")
    active = models.BooleanField(
        default=False, help_text="Is this service active or has it been decomissioned?"
    )
    description = models.TextField(blank=True, help_text="Longer description if needed")
    documentation = models.URLField(
        blank=True, help_text="URL to documentation for service in opman"
    )
    externally_visible = models.BooleanField(
        default=False,
    ...
    availability_tolerance = models.CharField(
        max_length=50,
        choices=(
    ...
    installer = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="service_installer",
        help_text="CEDA Person installing the service",
    )
    software_contact = models.ForeignKey(
        Person,
        on_delete=models.PROTECT,
        null=True,
        blank=True,
        related_name="service_software_contact",
...
class HostHistory(models.Model):
    ...
    history_desc = models.TextField(
        help_text="Details of event / noteworthy item in host's history"
    )
    admin_contact = models.ForeignKey(
        Person, on_delete=models.PROTECT, help_text="CEDA Person reporting this event"
    )
...
class ServiceBackupLog(models.Model):
    ...
    service = models.ForeignKey(
        Service, on_delete=models.PROTECT, help_text="Service being backed up"
    )
    backup_policy = models.ForeignKey(
        BackupPolicy,
        on_delete=models.PROTECT,
        help_text="Backup policy implemented for this backup event",
    )
    date = models.DateTimeField(help_text="Date and time of backup")
    success = models.BooleanField(
        default=False, help_text="Success (True) or Failure (False) of backup event"
    )
...
class FileSetSizeMeasurement(models.Model):
    ...
    fileset = models.ForeignKey(
        FileSet, on_delete=models.PROTECT, help_text="FileSet that was measured"
    )
    date = models.DateTimeField(
        default=datetime.now, help_text="Date and time of measurement"
    )
    ...
    no_files = models.BigIntegerField(
        null=True, blank=True, help_text="Number of files"
    )
...
class Audit(models.Model, ProblemsMixin):
    ...
    fileset = models.ForeignKey(
        FileSet,
        help_text="FileSet which this audit related to at time of creation",
    ...
    def fileset_link(self):
        return '<a href="/admin/cedainfoapp/fileset/%s">%s</a>' % (
            self.fileset.pk,
            self.fileset,
        )
    ...
    def prev_audit(self):
        ...
        audit = Audit.objects.filter(
            fileset=self.fileset, starttime__lt=self.starttime
        ).order_by("-starttime")
        if len(audit) == 0:
            return None
        else:
    ...
    def checkm_log(self):
        ...
        self.logfile = "%s/%s/checkm.%s.%s.log" % (
            settings.CHECKM_DIR,
            fileset.storage_pot,
            fileset.storage_pot,
    ...
    def _checkm_log(self, directory, storage_path, LOG):
        ...
        for n in names:
            ...
            if os.path.isfile(path) and not os.path.islink(path):
                ...
                mtime = datetime.utcfromtimestamp(os.path.getmtime(path)).strftime(
                    "%Y-%m-%dT%H:%M:%SZ"
                )
                F = open(path)
                m = hashlib.md5()
    ...
    def verify_copy(self):
        ...
        while 1:
            ...
            if m.hexdigest() != checksum:
                raise "checksums do not match %s" % newpath
    ...
    def problems():
        ...
        audits = Audit.objects.filter(
            auditstate="started", starttime__lt=time_threshold
        )
        for a in audits:
...
class GWSRequest(models.Model):
    ...
    gws_name = models.CharField(
        ...
        validators=[
            RegexValidator(
                ...
                message="Invalid name : this string will be used as a unix group name so must match the pattern %s"
                % settings.GWS_NAME_REGEX.pattern,
            )
        ],
        help_text="short string to be used as name for group workspace, also used for corresponsing unix group name. Must match pattern %s"
        % settings.GWS_NAME_REGEX.pattern,
    )
    path = models.CharField(
        max_length=2048,
        help_text="storage path to this group workspace excluding GWS name",
        choices=settings.GWS_PATH_CHOICES,
    )
    internal_requester = models.ForeignKey(
        User, on_delete=models.PROTECT, help_text="CEDA person sponsoring the request"
    )
    gws_manager = models.CharField(
        max_length=1024,
    ...
    description = models.TextField(
        null=True, blank=True, help_text="Text description of proposed GWS"
    )
    requested_volume = FileSizeField(
        help_text="In bytes, but can be enetered using suffix e.g. '200TB'", default="0"
    )
    # et_quota uses requested_volume as default unless other value specified: doesn't have to be the same)
    et_quota = FileSizeField(
        help_text="Elastic Tape quota. In bytes, but can be entered using suffix e.g. '200TB'",
        default="0",
    )
    backup_requirement = models.CharField(
        max_length=127, choices=settings.GWS_BACKUP_CHOICES, default="no backup"
    )
    related_url = models.URLField(
        blank=True, help_text="Link to further info relevant to this GWS"
    )
    expiry_date = models.DateField(
        default=datetime.now() + timedelta(days=2 * 365),
        help_text="date after which GWS will be deleted",
    )  # approx 2 years from now
    request_type = models.CharField(
        max_length=127,
        choices=settings.GWS_REQUEST_TYPE_CHOICES,
        default="new",
        help_text="type of request",
    )
    request_status = models.CharField(
        max_length=127,
        choices=settings.GWS_REQUEST_STATUS_CHOICES,
        default="pending",
        help_text="status of this request",
    )
    gws = models.ForeignKey(
        "GWS",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    ...
    timestamp = models.DateTimeField(
        auto_now=True, auto_now_add=False, help_text="time last modified"
    )
    ...
    def convert(self):
        ...
        if self.request_type == "new" and (self.gws is None or self.gws == ""):
            ...
            raise Exception(
                "Must set request status to update if updating an existing GWS"
            )

    def action_links(self):
        if self.request_status == "ceda pending":
            return format_html(
                '<a href="/gwsrequest/%i/approve">approve</a> <a href="/gwsrequest/%i/reject">reject</a>'
                % (self.id, self.id)
            )
    ...
    def gws_link(self):
        if self.gws_id is not None:
            return format_html(
                '<a href="/admin/cedainfoapp/gws/%i">%s%s</a>'
                % (self.gws.id, self.gws.path, self.gws.name)
            )
...
class GWS(models.Model):
    ...
    internal_requester = models.ForeignKey(
        User, on_delete=models.PROTECT, help_text="CEDA person sponsoring the GWS"
    )
    gws_manager = models.CharField(
        max_length=1024,
        help_text="External person who will manage the GWS during its lifetime",
    ...
    # et_quota uses requested_volume as default unless other value specified: doesn't have to be the same)
    et_quota = FileSizeField(
        help_text="In bytes, but can be entered using suffix e.g. '200TB'", default="0"
    )

    description = models.TextField(
        null=True, blank=True, help_text="Text description of GWS"
    )
    requested_volume = FileSizeField(
        help_text="In bytes, but can be entered using suffix e.g. '200TB'", default="0"
    )
    ...
    last_reviewed = models.DateTimeField(
        null=True, blank=True, help_text="date of last review"
    )
    review_notes = models.TextField(blank=True, help_text="notes from reviews (append)")
    status = models.CharField(
        max_length=127,
        choices=settings.GWS_STATUS_CHOICES,
        default="active",
        help_text="status of GWS",
    )
    et_used = FileSizeField(
        help_text="In bytes, but can be entered using suffix e.g. '200TB'", default="0"
    )
    ...
    def get_current_gwsrequest(self):
        ...
        try:
            gws = GWSRequest.objects.filter(
                gws=self, request_status="approved"
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk is None:
            ...
            raise Exception(
                "Unable to save changes to existing GWS : create an update request & get it approved"
            )
    ...
    def df(self):
        ...
        if os.path.isdir(gws_dir):
            # find volume using du
            output = subprocess.Popen(
                ["/usr/bin/df", "-k", gws_dir], stdout=subprocess.PIPE
            ).communicate()[0]
            lines = output.split("\n")
            if len(lines) == 3:
            ...
            # find number of files
            p1 = subprocess.Popen(
                ["find", gws_dir, "-type", "f"], stdout=subprocess.PIPE
            )
            p2 = subprocess.Popen(["wc", "-l"], stdin=p1.stdout, stdout=subprocess.PIPE)
            p1.stdout.close()  # Allow p1 to receive a SIGPIPE if p2 exits.
            nfiles = p2.communicate()[0]

            gwssm = GWSSizeMeasurement(
                gws=self, date=datetime.utcnow(), size=size, no_files=int(nfiles)
            )
            gwssm.save()
    ...
    def pan_df(self):
        ...
        if os.path.isdir(gws_dir):
            ...
            output = subprocess.Popen(
                ["/usr/local/bin/pan_df", "-k", gws_dir], stdout=subprocess.PIPE
            ).communicate()[0]
            lines = output.split("\n")
            if len(lines) == 4:
...
class GWSSizeMeasurement(models.Model):
    ...
    gws = models.ForeignKey(
        "GWS", on_delete=models.CASCADE, help_text="GWS that was measured"
    )
    date = models.DateTimeField(
        default=datetime.now, help_text="Date and time of measurement"
    )
...
class VMRequest(models.Model):
    vm_name = models.CharField(
        max_length=127, help_text="proposed fully-qualified host name"
    )  # TODO : need regex
    type = models.CharField(
        max_length=16, choices=settings.VM_TYPE_CHOICES, help_text="Type of VM, see REF"
    )  # TODO update REF
    operation_type = models.CharField(
        max_length=127,
        choices=settings.VM_OP_TYPE_CHOICES,
        help_text="Operation type of VM (dev, test, production, ...)",
    )
    internal_requester = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
        help_text="CEDA person sponsoring the request",
        related_name="vmrequest_internal_requester_user",
    )
    description = models.TextField(help_text="")
    date_required = models.DateField()
    cpu_required = models.CharField(
        max_length=127, choices=settings.VM_CPU_REQUIRED_CHOICES
    )
    memory_required = models.CharField(
        max_length=127, choices=settings.VM_MEM_REQUIRED_CHOICES
    )
    disk_space_required = models.CharField(
        max_length=127, choices=settings.VM_DISK_SPACE_REQUIRED_CHOICES
    )
    disk_activity_required = models.CharField(
        max_length=127, choices=settings.VM_DISK_ACTIVITY_REQUIRED_CHOICES
    )
    mountpoints_required = MultiSelectField(
        max_length=2048, choices=settings.MOUNT_CHOICES
    )
    network_required = models.CharField(
    ...
    patch_responsible = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
        related_name="vmrequest_patch_responsible_user",
    ...
    request_type = models.CharField(
        max_length=127, choices=settings.VM_REQUEST_TYPE_CHOICES, default="new"
    )
    request_status = models.CharField(
        max_length=127, choices=settings.VM_REQUEST_STATUS_CHOICES, default="pending"
    )
    ...
    )  # approx 3 years from now
    timestamp = models.DateTimeField(
        auto_now=True, auto_now_add=False, help_text="time last modified"
    )

    tenancy = models.ForeignKey(
        Tenancy, on_delete=models.PROTECT, null=True, blank=True
    )
    ...
    def action_links(self):
        if self.request_status == "ceda pending":
            return format_html(
                '<a href="/vmrequest/%i/approve">approve</a>|<a href="/vmrequest/%i/reject">reject</a>'
                % (self.id, self.id)
            )
        elif self.request_status == "ceda approved":
    ...
    def convert(self):
        ...
        if self.request_type == "new":  # and (self.vm is None or self.vm == ''):
            ...
            raise Exception(
                "Must set request status to update if updating an existing VM"
            )

    def vm_link(self):
        if self.vm is not None:
            return format_html(
                '<a href="/admin/cedainfoapp/vm/%i">%s</a>' % (self.vm.id, self.vm.name)
            )
...
class VM(models.Model):
    name = models.CharField(
        max_length=127, help_text="fully-qualified host name", unique=True
    )  # TODO : need regex
    type = models.CharField(
        max_length=16, choices=settings.VM_TYPE_CHOICES, help_text="Type of VM, see REF"
    )  # TODO update REF
    ...
    os_required = models.CharField(
        max_length=127, choices=settings.VM_OS_REQUIRED_CHOICES, default="rhel7"
    )
    other_info = models.TextField(blank=True)
    patch_responsible = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
        related_name="vm_patch_responsible_user",
        limit_choices_to={"id__in": settings.ADMIN_USERS_PATCH_RESPONSIBLE},
    )
    root_users = models.ManyToManyField(User, related_name="vm_root_users_user")
    status = models.CharField(
        max_length=127, choices=settings.VM_STATUS_CHOICES, default="active"
    )
    ...
    def action_links(self):
        return format_html(
            '<a href="/vm/%i/changestatus">change status</a>' % (self.id,)
        )
    ...
    def service_count(self):
        production_count = len(
            NewService.objects.filter(host_id=self.id, status="production")
        )
        decomissioned_count = len(
            NewService.objects.filter(host_id=self.id, status="decomissioned")
        )

        return '<a href="/admin/cedainfoapp/newservice/?Host=%s">%s (%s)</a>' % (
            self.id,
            production_count,
            decomissioned_count,
        )
...
class NewService(models.Model):
    ...
    uptimerobot_monitor_id = models.IntegerField(
        help_text="ID number of UptimeRobot monitor for this service", blank=True
    )
    status = models.CharField(
        max_length=50,
        choices=(
    ...
    service_manager = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="software_manager",
        help_text="CEDA person who looks after this service",
    )

    deputy_service_manager = models.ForeignKey(
        Person,
        on_delete=models.PROTECT,
        null=True,
        blank=True,
        related_name="deputy_service_manager",
    ...
    owner = models.ForeignKey(
        ...
        blank=True,
        related_name="owner",
        help_text="Owner of this service",
    )

    startdate = models.DateField(
        null=True, blank=True, help_text="Approximate date service went into production"
    )
    last_reviewed = models.DateField(
        null=True, blank=True, help_text="Date of last review"
    )
    ...
    ports = models.CharField(
        max_length=100,
        blank=True,
        help_text="External open ports required by this service",
    )
