    ...
    for f in filesets:
        ...
        if not first_time:
            ...
            if len(fssms) < 10 and not f.sd_backup:
                print_msg(f, "Newish and not marked for backup.")
        else: 
            print_msg(f, "Not measured yet")
            continue

        if too_many_files:
            print_msg(f, "Too Many files")
        if too_big:
            print_msg(f, "Too Big")
        if over_alloc:
            print_msg(f, "Over allocation")

        if f.sd_backup:
            backup_processed = f.sd_backup_process_log()[-13:-5]
            if backup_processed[:3] == "Not":
                print_msg(f, "Not processed for backup yet")
            else:
...
def print_msg(obj, msg):
    """print a message about an object with a link to admin interface"""
