...
def run(*script_args):
    ...
    with open(vm_file) as csv_file:
        ...
        for row in csv_reader:
            ...
            vm_visibility = row[2]
            vm_type = row[3]
            vm_operation_type = row[4]
            vm_description = row[5]
            vm_location = row[6]

            vm_cpu  = row[7]
            vm_ram  = row[8]
            vm_disk = row[9]
            vm_disk_activity = row[10]
            vm_mount_points = row[11]
            vm_location = row[12]
            vm_network_activity = row[13]
...
#                print "Creating: %s" % vm_name
       ...
       #         vm_operation_type = 'production'
                ...
                other_info += 'Mountpoints requested: ' +  vm_mount_points
                ...
                a= VM(
                    ...
                    disk_activity_required=vm_disk_activity,
                    mountpoints_required='',
                    network_required = vm_network_activity,
                    os_required='centos7',
                    status= vm_status,
